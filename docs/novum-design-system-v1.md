...
**Inteligencia del sector.** Editorial + analítica + tecnológica, nunca un dashboard: cabecera con la geometría propia de la sección (anillos y nodos del lenguaje del hero, decorativa, solo en escritorio), números grandes con mucho aire (`display-sm`/`display-md`, `tnum`, unidad en línea aparte), y tres historias en filas alternas: visualización a un lado (7/12), lectura ejecutiva al otro (5/12) con filete morado; abajo universo · metodología · fuente en `small`. Visualizaciones sin librería: barras SVG mínimas (`viewBox 0 0 100 10`, `preserveAspectRatio="none"`) con etiquetas y valores en HTML siempre visibles, `<title>` por barra, tabla `sr-only` equivalente; paleta morada por orden (900 → 700 → 500) + `green-500` solo como relleno y `green-700` para la variación en texto. Sin snapshot: tres capítulos editoriales (numeral, título, pregunta en `lead` morado, texto), sin gráficas ni placeholders. «Su entidad en contexto» cierra la sección con el glifo de dos puntos sobre una escala y el CTA de demostración.
